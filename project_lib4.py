...
#results of parameter optimisation per strategy (see notebook optimization_parameters_ATS)
global LTMA, MTMA, STMA, SSTMA, SMACD, LMACD, RSI, BB
LTMA= {'CO1': 160, 'SP1': 200, 'TY1': 210, 'HG1': 161, 'NQ1': 544, 'GC1': 538}
MTMA= {'CO1': 75, 'SP1': 129, 'TY1': 130, 'HG1': 80, 'NQ1': 63, 'GC1': 45}
...
LMACD= {'CO1': (44, 732), 'SP1': (348, 437), 'TY1': (264, 641), 'HG1': (460, 505), 'NQ1': (100, 141), 'GC1': (625, 681)}
RSI= {'CO1': (1251, 30), 'SP1': (531, 30), 'TY1': (33, 30), 'HG1': (1043, 30), 'NQ1': (636, 30), 'GC1': (114, 30)}
BB= {'CO1': (2, 2.), 'SP1': (487, 2.), 'TY1': (134, 2.), 'HG1': (243, 2.), 'NQ1': (1168, 2.), 'GC1': (85, 2.)}
...
def feature(df,start_date,end_date):
	...
	# more features
    df['mtma']=df.close.rolling(MTMA[name]).mean()
    df['stma']=df.close.rolling(STMA[name]).mean()
    df['ltma']=df.close.rolling(LTMA[name]).mean()
    df['sstma']=df.close.rolling(SSTMA[name]).mean()
    df['smacd1']=df.close.rolling(SMACD[name][0]).mean()
    df['smacd2']=df.close.rolling(SMACD[name][1]).mean()
    df['lmacd1']=df.close.rolling(LMACD[name][0]).mean()
    df['lmacd2']=df.close.rolling(LMACD[name][1]).mean()
    df['stdbb'] = df.close.rolling(BB[name][0]).std() 
    df['bbma'] = df.close.rolling(BB[name][0]).mean() 
    df['bollu'] = df['bbma'] + BB[name][1]*df['stdbb'] 
    df['bolld'] = df['bbma'] - BB[name][1]*df['stdbb']
    df['rsi'] = rsi(df.close,RSI[name][0])
...
def signal(df):
    ...
    s=pd.DataFrame()    
    s['mtma']=-np.array(df.mtma>df.close).astype(int)+np.array(df.mtma<df.close).astype(int)
    s['stma']=-np.array(df.stma>df.close).astype(int)+np.array(df.stma<df.close).astype(int)
    s['ltma']=-np.array(df.ltma>df.close).astype(int)+np.array(df.ltma<df.close).astype(int)
    s['lmacd']=np.array(df.lmacd1>df.lmacd2).astype(int)-np.array(df.lmacd1<df.lmacd2).astype(int)
    s['smacd']=np.array(df.smacd1>df.smacd2).astype(int)-np.array(df.smacd1<df.smacd2).astype(int)
    #s['1d']=np.array(df.close>df.close.shift(1)).astype(int)-np.array(df.close<df.close.shift(1)).astype(int)
    s['Bollinger']=np.array(df.close<df.bolld).astype(int)-np.array(df.close>df.bollu).astype(int)
    s['rsi']=np.array(df['rsi']<RSI[name][1]).astype(int)-np.array(df['rsi']>100-RSI[name][1]).astype(int)
